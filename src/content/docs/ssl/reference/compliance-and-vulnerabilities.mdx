...
description: Configure your Cloudflare zone for PCI DSS compliance — set minimum TLS version, configure approved cipher suites, enable TLS 1.3, and understand how Cloudflare interacts with PCI ASV scans.
products:
  - ssl
...
import { DashButton, TabItem, Tabs, APIRequest } from "~/components";
...
:::note
You cannot configure specific TLS 1.3 cipher suites. When you [enable TLS 1.3](#step-3-enable-tls-13), Cloudflare automatically uses all supported TLS 1.3 cipher suites, all of which meet PCI DSS requirements. The cipher list in this step applies to TLS 1.2 connections only.
:::
...
[SSL Labs Server Test](https://www.ssllabs.com/ssltest/) gives an external view of your TLS configuration. Enter your domain and check:

- **Protocol support**: TLS 1.2 and TLS 1.3 only — no TLS 1.0 or TLS 1.1
- **Cipher strength**: No weak or deprecated cipher suites listed
...
Cloudflare proxies a [defined set of HTTP and HTTPS ports](/fundamentals/reference/network-ports/). Connections to ports outside that list are not proxied, and either reach your origin directly or are refused, depending on your origin configuration.
...
Cloudflare applies the following mitigations by default across all proxied zones. No configuration is needed.
...
If a scanner flags Sweet32, verify that TLS 1.0 is disabled on your zone (see [Step 1](#step-1-set-minimum-tls-version-to-12)). With TLS 1.0 disabled, the 3DES cipher suites where Sweet32 applies are not in use and the finding does not apply to your environment.
