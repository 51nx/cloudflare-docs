...
| Business justification | A written justification (business or technical) for each script | You can review and annotate each script in the dashboard to document its purpose |
| Authorization method | A method to confirm that each script is authorized | Rules and integrity hash checks detect scripts that were not explicitly authorized |
...
| Control objective | What it requires | How client-side security addresses it |
| --- | --- | --- |
| Header tamper detection | Detect unauthorized changes to HTTP security headers | Alerts fire when monitored headers (such as `Content-Security-Policy` or `X-Frame-Options`) change unexpectedly |
| Content tamper detection | Detect unauthorized changes to page content | Alerts fire when scripts, connections, or cookies on a monitored page change |
| Detection frequency | Detection must run at least once every seven days | Cloudflare's detection runs continuously — no manual scheduling is required |
